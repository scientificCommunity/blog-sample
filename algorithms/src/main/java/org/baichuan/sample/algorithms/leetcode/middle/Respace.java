...
public class Respace {
     ...
     * 状态转移方程: dp[i+1]=min(dp[idx], dp[i+1])
     * 解释: 当i向右移动一位，有两种情况：
     * 1. 不去匹配第i+1个字符（不管字典树里有没有这个字，都不去匹配。此时如果恰好存在一种情况：从n到i+1组成的字符被匹配到了(n为dp[i]里以i结尾的未匹配的连续字符串)，则这种情况可以由第二种转移方式处理。），则dp[i+1] = dp[i] + 1
     * 2. 匹配第i+1个字符，此时有可能匹配到多个以该字符结尾的词（若没匹配到，则为第一种情况）。此时可以得到一个数组，数组中的项idx为匹配到的词的第一个字符在sentence中的位置。
     * 此时，对每个idx对应的组成方式求最小值：min(dp[idx])，然后再拿这个最小值跟第一种情况做对比并取较小值。
